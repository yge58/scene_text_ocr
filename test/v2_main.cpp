...
int main(int args, char **argv)
{
  ...
  while (1)
      ...
      else if ( key == ' ' ) // if entered space key  
	{
	  ...
	  // Any direction.
	   erGrouping(frame, channels, regions, region_groups, boxes, ERGROUPING_ORIENTATION_ANY, "./trained_classifier_erGrouping.xml", 0.7);

	  cout << ">>>>>>>> \tGrouping text regions done! \n";
	  ...
	  cout << ">>>>>>>>> \tSorting text regions done! \n";
	  ...
	  for ( auto it : boxes_list )
	    {
	      cout << it << endl;
	      // Mat tmp = Mat::zeros(it.width + 2, it.height + 2, CV_8UC3);
	      //frame(it).copyTo(tmp);    
	      // imshow( "textBox_" + to_string(i++), frame(it) );
	    }
